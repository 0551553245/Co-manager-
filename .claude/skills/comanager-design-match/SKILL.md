...
| Manager dashboard "Shift ends: 22:00" + shift progress ring | Was mockup filler when first reviewed (2026-07-26) — correctly ignored then, no shift system existed at that point. **Reversed 2026-08-05**: Work Shifts is now a real, deliberately-scoped feature (owner-defined per-branch shifts, manual shift switcher, handover notes) — but NOT the fixed "22:00"/progress-ring version this row originally shown here; see comanager-logic §9 for the actual locked architecture. | comanager-logic §9 (supersedes this row's original "don't build a shift system" resolution) |
